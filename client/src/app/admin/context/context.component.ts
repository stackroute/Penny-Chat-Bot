  ...
  submitContext() {
    if(this.setDomain != undefined) {
      if((this.setDomain == true && this.selectedContext.name != Config.resetDomain.label) || this.setDomain == false) {
        ...
        .subscribe((ref) => {
          if(ref.status == true){
            swal("Context added successfully",
                'success'
              )
            this.context = {};
            this.synonym = [];
            ...
            swal("error in creating a context node",
                'warning'
              )
          }
        })
        ...
        swal("Please add dependent context first",
                'warning'
              )
      }
    }
    else {
      swal("Please select any Option Domain/Add to Context",
                'warning'
              )
    }
  }
